...
const Snacksitems = (props) => {
  ...
  const {id} = useParams();
  const snacksItems = props.snacksItems.find((snacksItems) => snacksItems.id === id)
  ...
  return (
    ...
    <div key={snacksItems.id} className="wrap-cover">
      <div className='display-cover'>
        <div className='image-display-cover'>
          <img src={snacksItems.cover} alt={snacksItems.name}/>
        </div>
        <div className='product-name-price'>
          <h2>{snacksItems.name}</h2>
          <p>Shop {snacksItems.name} now with just a click of a button below from Chuka connect</p>
          <button onClick={() => props.addToCart(snacksItems)}>Add to cart(Ksh {snacksItems.price})</button>
          <h1>Delivery of {snacksItems.name} will just take a few minutes upon confirming checkout from the cart</h1>
...
<div className='product-description'>
  ...
  <p>{snacksItems.description}</p>
</div>
<div className='product-specification'>
  <h2>Product Specification</h2>
  <p>{snacksItems.specifications}</p>
</div>
</div>
